		      ...
		      struct fuse_config *cfg)
{
	...
	//Connect to server
	ctx->client_fd = connect_server(1);
	ctx->client_fd2 = connect_server(2);
		       ...
		       struct fuse_file_info *fi)
{
	...
	struct stat staux;
	int remote_res = rstat(p_ctx->client_fd, p_ctx->client_fd2, path, &staux);
	if(remote_res==0){
		    ...
		    struct fuse_file_info *fi)
{
	...
	//TODO ex2
	res = rpread(p_ctx->client_fd, path, buf, size, offset);
		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	res = rpwrite(p_ctx->client_fd, p_ctx->client_fd2, path, buf, size, offset);
